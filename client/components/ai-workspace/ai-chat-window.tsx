...
interface AIMessage {
  ...
  timestamp: Date;
  suggestions?: string[];
  actionType?: 'code_generation' | 'file_creation' | 'error_analysis' | 'improvement';
}
...
export const AIChatWindow = forwardRef<HTMLDivElement, AIChatWindowProps>(({
  ...
  const [messages, setMessages] = useState<AIMessage[]>([
    {
      id: '1',
      content: "Hi! I'm Josey AI, your development assistant. I can help you generate code, analyze errors, create files, and guide you through the development process. What would you like to build today?",
      isUser: false,
      timestamp: new Date(),
      suggestions: [
  ...
  const getActionIcon = (type?: string) => {
    switch (type) {
      case 'code_generation': return <Code className="h-3 w-3" />;
      case 'file_creation': return <FileText className="h-3 w-3" />;
      case 'error_analysis': return <Zap className="h-3 w-3" />;
